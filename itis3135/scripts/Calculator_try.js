...
var numClick = function(event) {
  ...
  if (document.getElementById("calc-display").innerHTML == "0") {
    document.getElementById("calc-display").innerHTML = event.target.innerHTML;
  // If display ends with .0, remove 0 and then concatenate
    ...
    document.getElementById("calc-display").innerHTML = document.getElementById("calc-display").innerHTML.slice(0, -1) + event.target.innerHTML;
...
var opClick = function(event) {
  // Add display value to operand array
...
var equalsClick = function() {
  ...
  for (let i = 0; i < operands.length; i++) {
    var nextOperand;
    // Use display value as the last operand, otherwise use the next operand in the array
    ...
    // Perform calculation based on the current operation symbol
    if (operations[i] == "+") {
  ...
  // Set the final total to the display value
